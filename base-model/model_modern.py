...
# get the absolute path between folder that contains part_2 and part_3
import os, sys
...
class GPTModern(nn.Module):
    ...
    def forward(
        self,
        idx: torch.Tensor,
        targets: torch.tensors | None = None,
        kv_cache_list=None,
        start_pos: int = 0,
    ):
        B, T = idx.shape
    ...
    def generate(
        ...
        for _ in range(max_new_tokens):
            # feed full content once; then only the last token
            idx_cond = idx[:, -self.block_size :] if kvs[0] is None else idx[:, -1:]

            # absolute cache position from cache length else (0 on the first step)
            ...
            next_id = (
                ...
                if temperature == 0.0
                else torch.multinomial(probas, 1)
            )
    ...
    def generate_nocache(
        ...
        for _ in range(max_new_tokens):
            # always run a full forward
            idx_cond = idx[:, -self.block_size :]

            # absolute position of the first token in the window
            ...
            next_logits = _tk(next_logits, top_k=top_k, top_p=top_p)
            probas = torch.softmax(next_logits, dim=-1)
            top_v, top_ids = torch.topk(probas, 10)
            # print("top ids : ", top_ids.tolist())
            # print("top vs : ", top_v.tolist())
            next_id = (
                torch.argmax(probas, dim=-1, keepdim=True)
                if temperature == 0.0
                else torch.multinomial(probas, 1)
