...
class BPETokenizer:
    ...
    Trains on a text file or a folder of .txt files. Saves merges/vocabs to out_dir
    """
    ...
    def train(self, data_path: Union[str, Path]):
        ...
        p = Path(data_path)
        if p.is_dir():
            files = [str(pf) for pf in p.glob("**/*.txt")]
        else:
            files = [str(p)]
    ...
    def load(self, dir_path: Union[str, Path]):
        ...
        # ṭok = ByteLevelBPETokenizer(str(vocab),str(merges))
