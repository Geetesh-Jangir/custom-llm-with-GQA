...
def _maybe_log_attention(logger, model, xb, step: int, every: int = 100):
    ...
    try:
        ...
        with torch.no_grad(), torch.cuda.amp.autocast(enabled=False):
            # recreate the inputs seen by blocks
            x = model.tok_emb(xb)
            x = model.drop(x)
...
def _extract_config_from_model(model) -> dict:
    ...
    try:
        ...
        cfg["rope"] = getattr(attn, "use_rope", getattr(attn, "rope", True))
        # positional / attention tricks
        for k in ("max_pos", "sliding_window", "attention_sink"):
            if hasattr(attn, k):
                val = getattr(attn, k)
...
def _verify_model_matches(model, cfg: Dict[str, Any]) -> Tuple[bool, str]:
    ...
    got.update(
        {
            "n_head": int(first_block.attn.n_head),
            "n_embd": int(first_block.attn.n_head * first_block.attn.d_head),
            "n_kv_head": int(
                getattr(first_block.attn, "n_kv_head", first_block.attn.n_head)
            ),
        }
    )
...
def save_checkpoint(
    ...
    # prefer the models config if available
...
def load_checkpoint(
    ...
    if cfg:
        ...
        if not ok:
            raise RuntimeError(
                msg + "\nRebuild the model with the config, or load with strict=False"
            )
    else:
        print("Warning : checkpoint has no config; cannot verify architecture")
