...
def run_cfg_from_args(args, vocab_size: int):
    return dict(
...
def main():
    ...
    p.add_argument("--bpe", action="store_true", help="train and use BPE tokenizer")
    ...
    p.add_argument(
        ...
        type=int,
        default=50,
        help="save checkpoint after every n optimizer steps",
    )
    p.add_argument(
        "--keep_last_k",
        type=int,
        default=2,
        help="keep last k model checkpoints + model_last.pt",
    )
    args = p.parse_args()

    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")

    # out_dir and (possible) checkpoints
    ...
    if have_ckpt:
        ...
        if "config" not in ckpt:
            raise RuntimeError(
                ...
                "Please re-save the a checkpoint that includes the model config."
            )
    ...
    if have_ckpt:
        ...
        print(f"[Resume] Loaded tokenizer from {tok_dir} (vocab_size={vocab_size})")
    else:
        if args.bpe:
    ...
    # init model scheduler optimizer amp
    ...
    # graceful save on sigint and sigterm
